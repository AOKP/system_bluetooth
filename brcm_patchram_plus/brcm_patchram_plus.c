...
int parse_bdaddr1(char*);
int parse_bdaddr2(char*);
int parse_bdaddr3(char*);
...
parse_bdaddr(char *optarg)
{
	if ( strstr( optarg, ":") )
	{
		/* address is separated by : characters .. use parse_bdaddr1() */
		return parse_bdaddr1(optarg);
	}
	else if ( strstr( optarg, "-" ) )
	{
		/* address is separated by - characters .. use parse_bdaddr3 */
		return parse_bdaddr3(optarg);
	}
	else
	{
		/* hope the address doesn't have separaters .. use parse_bdaddr2 */
		return parse_bdaddr2(optarg);
	}
}


int
parse_bdaddr1(char *optarg)
{
	int bd_addr[6];
	int i;
	int matches;

	matches = sscanf(optarg, "%02X:%02X:%02X:%02X:%02X:%02X",
			 &bd_addr[5], &bd_addr[4], &bd_addr[3],
			 &bd_addr[2], &bd_addr[1], &bd_addr[0]);

	/* only do anything if we successfully parsed the mac address */
	if ( matches == 6 )
	{
		for (i = 0; i < 6; i++) {
	...
	{
		fprintf( stderr, "Invalid mac address - ignoring.\n")
	}
...
read_default_bdaddr()
{
	...
	memset(addr_from_ril, 0, sizeof(addr_from_ril));
 ...
 */
	...
	if (path[0] != 0)
	{
		...
		sz = read(fd, bdaddr, sizeof(bdaddr) - 1);
		if (sz < 0) {
		    fprintf(stderr, "read(%s) failed: %s (%d)", path, strerror(errno),
				errno);
		    close(fd);
		    return;
		} else if (sz != (sizeof(bdaddr)-1) && sz != 12) {
